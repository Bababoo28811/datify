// icons.js — inline SVG icons, generated from lucide-static v1.47.0 (ISC licence).
// Regenerate rather than hand-editing. Emoji were previously used for these:
// they render differently on every platform, ignore currentColor, and don't
// share a stroke weight, so nothing lined up optically.
//
// icon(name) returns an <svg> string. Icons are decorative by default
// (aria-hidden) because every one sits next to visible text that already
// says what it means. Pass a label only when the icon stands alone.
(function () {
  const PATHS = {
  ...
  window.icon = function (name, opts) {
    ...
    return `<svg class="${cls}" width="${size}" height="${size}" viewBox="0 0 24 24" `
      ...
      + `stroke-linejoin="round" ${a11y}>${p}</svg>`;
  };

  // Fills any <el data-icon="name"> in static markup, so index.html can stay
  // declarative instead of being rebuilt in JS. data-icon-size overrides 16.
  window.hydrateIcons = function (root) {
    (root || document).querySelectorAll('[data-icon]').forEach(function (el) {
      if (el.dataset.iconDone) return;
      var svg = window.icon(el.dataset.icon, { size: +el.dataset.iconSize || 16 });
      if (!svg) return;
